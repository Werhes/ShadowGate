...
pub async fn connect_direct_ws(
    ...
    if domains.is_empty() { return (None, false, false); }
        ...
        match ws_connect(target, dom, "/apiws", timeout).await {
            ...
            Err(e) => {
                ...
                if let Some(h) = e.handshake() {
                    if h.is_redirect() { ws_failed_redirect = true; }
                    else { all_redirects = false; }
                } else { all_redirects = false; }
            }
        }
    }
...
pub async fn run_proxy(
...

    {
        ...
        tokio::spawn(async move { p.warmup(&map).await; });
    }
...
pub fn parse_cidr_pool(cidrs_str: &str) -> HashMap<i32, String> {
    ...
    if cidrs_str.trim().is_empty() { return result; }
...
pub async fn handle_client(pool: Arc<WsPool>, mut conn: TcpStream, cancel_token: CancellationToken) {
    ...
    loop {
        ...
        if relay_init[0] == 0xEF { continue; }
        let s = &relay_init[..4];
        if s == b"HEAD" || s == b"POST" || s == b"GET "
            || s == &[0xee, 0xee, 0xee, 0xee]
            || s == &[0xdd, 0xdd, 0xdd, 0xdd] { continue; }
        if relay_init[0] == 0x16 && relay_init[1] == 0x03
            && relay_init[2] == 0x01 && relay_init[3] == 0x02 { continue; }
        if relay_init[4] == 0 && relay_init[5] == 0
            && relay_init[6] == 0 && relay_init[7] == 0 { continue; }
        break;
    }

    // Telegram encryptor/decryptor
    ...
    // Если нет конфигурации DC или blacklist — сразу fallback
    if !dc_configured || blacklisted {
        do_fallback(conn, &relay_init, label, dc, is_media, splitter,
            &clt_decryptor, &clt_encryptor, &tg_encryptor, &tg_decryptor, cancel_token).await;
        return;
    }

    // Попытка прямого WS подключения
    let fail_until = DC_FAIL_UNTIL.read().get(&dc_key).copied().unwrap_or(0.0);
    let ws_timeout = if now < fail_until { WS_FAIL_TIMEOUT } else { 10.0 };
    ...
    if ws_opt.is_none() {
        ...
        do_fallback(conn, &relay_init, label, dc, is_media, splitter_fb,
            &clt_decryptor, &clt_encryptor, &tg_encryptor, &tg_decryptor, cancel_token).await;
        return;
    }

    // Отправляем relayInit через WS
...
fn log_cf_conn_error(msg: &str, err: &WsError) {
    if let Some(h) = err.handshake() {
    ...
    let down_task = tokio::spawn(async move {
        loop {
            ...
            *la_down.lock().await = std::time::Instant::now();
            ...
            if conn_write.write_all(&data).await.is_err() { break; }
        }
...
pub async fn bridge_tcp(
    ...
    let up = async move {
        ...
        loop {
            ...
            if r_write.write_all(chunk).await.is_err() { break; }
        }
        cancel_up.notify_waiters();
    };
    ...
    let down = async move {
        ...
        loop {
            ...
            if c_write.write_all(chunk).await.is_err() { break; }
        }
        cancel_down.notify_waiters();
    };
...
pub async fn tcp_fallback(
    ...
    if remote.write_all(init).await.is_err() { return false; }
    bridge_tcp(
        client, remote, label, dc, dst.to_string(), port, is_media,
        clt_dec, clt_enc, tg_enc, tg_dec, cancel_token,
    ).await;
    true
}
...
async fn try_cfproxy_base_domain(dc: i32, base_domain: &str) -> (Option<RawWebSocket>, String) {
    ...
    if remaining > Duration::ZERO {
        ldebug!(" CF skip {}: 429 cooldown {:.0}s", base_domain, remaining.as_secs_f64().ceil());
        return (None, String::new());
    }
    ...
    if let Some(e) = err {
        ...
        if !resolved_ip.is_empty() {
            log_cf_conn_error(&format!(" CF fail {} via {}: {}", domain, resolved_ip, e.compact()), &e);
        } else {
...
async fn cfproxy_acquire_ws(
    ...
    let (enabled, _active, domains) = {
        ...
        (CFPROXY_ENABLED.load(Ordering::Relaxed), cfg.active.clone(), cfg.domains.clone())
    };
    if !enabled || domains.is_empty() { return None; }
        ...
        for h in handles {
            if let Ok(Some((w, d))) = h.await {
                if ws.is_none() {
                    ...
                    tokio::spawn(async move { w.close().await; });
                }
            }
        }
    }
        ...
        if let Some((ws, chosen_domain)) = cfproxy_acquire_ws(dc, is_media, &cancel_token).await {
            ...
            if ws.send(relay_init).await.is_err() {
                ...
                if !fallback_dst.is_empty() {
                    return tcp_fallback(conn, &fallback_dst, 443, relay_init, label, dc, is_media,
                        clt_dec, clt_enc, tg_enc, tg_dec, cancel_token).await;
                }
                return false;
            }

            bridge_ws(conn, ws, label, dc, chosen_domain, 443, is_media,
                splitter, clt_dec, clt_enc, tg_enc, tg_dec, cancel_token).await;
            return true;
        }
    }
...
impl WsPool {
    ...
    pub async fn get(
        ...
        let slot = DcSlot { dc, is_media: is_media_int(is_media) };
                ...
                match q.pop_front() {
                    Some(entry) => {
                        if is_pool_entry_usable(&entry, now) {
                            ...
                            let e = entry;
                            tokio::spawn(async move { e.ws.close().await; });
                            continue;
                        }
                    }
                    None => {
    ...
    pub async fn warmup(self: &Arc<Self>, dc_opt_map: &HashMap<i32, String>) {
        for (dc, target_ip) in dc_opt_map {
            if target_ip.is_empty() { continue; }
            for is_media in [false, true] {
                let domains = ws_domains(*dc, is_media);
                let slot = DcSlot { dc: *dc, is_media: is_media_int(is_media) };
                let state = self.get_slot(slot).await;
                if state
                    .refilling
                    .compare_exchange(0, 1, Ordering::SeqCst, Ordering::SeqCst)
...
fn is_pool_entry_usable(e: &PoolEntry, now: i64) -> bool {
    if e.ws.is_closed() { return false; }
    if now - e.created > WS_POOL_REUSE_MAX_AGE as i64 { return false; }
    true
}
